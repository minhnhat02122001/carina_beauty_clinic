...
export default async function PromotionsPage({ params }: { params: Promise<{ locale: Locale }> }) {
  const { locale } = await params;
  const [t, posts] = await Promise.all([getTranslations("Promotions"), getPostsByCategory("promotions", locale)]);
    ...
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 md:px-10 lg:py-16">
      ...
      <div className="mt-8">
        <PostList
          posts={posts}
          emptyLabel={t("empty")}
          getHref={(slug) => ({ pathname: "/promotions/[slug]", params: { slug } })}
        />
      </div>
    </div>
  );
