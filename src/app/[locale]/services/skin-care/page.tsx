...
export default async function ServiceSkinCarePage({ params }: { params: Promise<{ locale: Locale }> }) {
  const { locale } = await params;
  const [t, treatments] = await Promise.all([
    getTranslations("ServiceSkinCare"),
    getTreatmentsByCategory("skin-care", locale),
  ]);
    ...
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 md:px-10 lg:py-16">
      ...
      <div className="mt-8">
        <TreatmentList
          ...
          emptyLabel={t("empty")}
          getHref={(slug) => ({ pathname: "/services/skin-care/[slug]", params: { slug } })}
        />
      </div>
    </div>
  );
