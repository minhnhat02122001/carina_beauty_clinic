...
export default async function SkinCareTreatmentDetailPage({
  params,
}: {
  ...
  const [t, treatment] = await Promise.all([
    getTranslations("ServiceSkinCare"),
    getTreatmentBySlug("skin-care", slug, locale),
  ]);
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6">
      <Link href="/services/skin-care" className="mt-8 inline-block text-sm font-bold text-[var(--color-link)]">
        {t("backToList")}
      </Link>
