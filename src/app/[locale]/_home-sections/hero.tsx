...
function Stat({ value, label, isMobileLeftColumn }: { value: string; label: string; isMobileLeftColumn: boolean }) {
  return (
    ...
    >
      <p className="text-2xl font-semibold text-[var(--foreground)] lg:text-[38px]">{value}</p>
...
function StatDivider() {
  return <div className="hidden bg-[rgba(99,43,14,0.2)] lg:block lg:h-[55px] lg:w-px" />;
}
    ...
    <section className="bg-[var(--color-background-alt)]">
      <div className="mx-auto flex max-w-[1440px] flex-col gap-8 px-4 py-8 sm:px-6 md:px-10 lg:grid lg:grid-cols-2 lg:items-center lg:gap-12 lg:px-28 lg:py-12">
        <div className="flex flex-col items-center gap-4 text-center lg:items-start lg:text-left">
          <span className="rounded-full bg-[rgba(201,169,97,0.16)] px-3 py-2 text-[10px] font-bold uppercase tracking-[2.4px] text-[var(--foreground)] lg:text-xs">
            {t("badge")}
          </span>
          ...
          <h1 className="font-script text-5xl leading-tight text-[var(--color-gold)] lg:text-[76px]">{t("scriptHeading")}</h1>
          ...
          <div className="grid grid-cols-2 gap-3 pt-2 lg:flex lg:gap-4">
            <Link
              href="/services"
              className="whitespace-nowrap rounded-full border-2 border-[var(--color-accent)] px-3 py-2 text-center text-xs font-bold text-[var(--color-accent)] hover:opacity-80 sm:px-4 sm:text-sm lg:px-[50px] lg:py-2.5 lg:text-base"
            >
              {t("ctaServices")}
            </Link>
            <Link
              href="/booking"
              className="whitespace-nowrap rounded-full border-2 border-[var(--color-accent)] bg-[#50260e] px-3 py-2 text-center text-xs font-bold text-[var(--color-background-alt)] hover:opacity-90 sm:px-4 sm:text-sm lg:px-[18px] lg:py-2.5 lg:text-base"
            >
              {t("ctaBooking")}
            </Link>
          </div>
          <div className="grid grid-cols-2 gap-x-2 gap-y-2 pt-4 lg:flex lg:items-center lg:gap-2">
